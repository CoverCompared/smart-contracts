...
describe("MSOPolka", function () {
  ...
  beforeEach(async function () {
    ...
    this.msoPolka = await (await this.MSOPolka.deploy(
      this.wethAddress,
  ...
  it("Should buy MSO by ETH", async function () {
    ...
    hexData =
      hexStr +
      paddedPriceUSDHexStr.slice(2) +
      paddedPeriodHexStr.slice(2) +
      paddedConciergePriceHexStr.slice(2);
    ...
    const expectedAmount = await this.exchangeAgent.getETHAmountForUSDC(
      priceUSD + conciergePrice
    );

    await expect(
      this.msoPolka.buyProductByETH(
        productName,
        priceUSD,
        productPeriod,
        conciergePrice,
        flatSig,
        { value: getBigNumber(5, 16) }
      )
    )
  ...
  it("Should buy MSO by available token", async function () {
    ...
    const expectedAmount = await this.exchangeAgent.getTokenAmountForUSDC(
      this.cvrAddress, priceUSD + conciergePrice
    );

    await this.cvr.connect(this.signers[0]).approve(this.msoPolka.address, getBigNumber(100000000000));

    const allowance = await this.cvr.allowance(this.signers[0].address, this.msoPolka.address);
